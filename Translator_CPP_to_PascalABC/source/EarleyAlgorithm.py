...
from source.LexicalAnalyzer import LexemeType
...
class Earley:
    ...
    def printTableToFile(self, tableType="ver"):
        with open("Output_EarleyAlgorithm.txt", "w+", encoding="utf-8") as file:
...
class TreeBuilder:
    ...
    def printTreeToFile(self):
        if self.tree is not None:
            with open("Output_ParseTree.txt", "w+", encoding="utf-8") as file:
                self.file = file
                self.__printTreeToFileHelper(self.tree)

    def printTreeToFileTest(self, trees):
        if trees is not None:
            with open("Output_ParseTree1.txt", "w+", encoding="utf-8") as file:
                self.file = file
                for tree in trees:
