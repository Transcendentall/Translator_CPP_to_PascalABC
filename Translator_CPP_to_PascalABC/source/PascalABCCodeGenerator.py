...
class VarNode(GeneratorNode):
    def __init__(self, parent, name=None, varType=None, body=None):
...
class Generator(object):
    ...
    def __parseFunc(self, treeNode, parentNode):
        ...
        for child in treeNode.children:
            if str(child) == '<тело функции>':
                child_found = count
            count += 1
        body = self.__parse(treeNode.children[child_found], result)
        result.addBody(body)
    ...
    def __parseFormalParams(self, treeNode, parentNode):
        if treeNode.children[1].lexeme.lexeme == ';' or treeNode.children[0].lexeme.lexeme == ';':
            result = []
        else:
            result = [
                VarNode(parentNode, treeNode.children[1].lexeme.lexeme, varType=treeNode.children[0].lexeme.lexeme)]
    ...
    def __parseExpression(self, treeNode, exprNode: ExpressionNode, parentNode):
        if treeNode.rule.name == '<вызов функции>':
            ...
            if treeNode.lexeme.lexeme == '==':
                exprNode.addBody('=')
            elif treeNode.lexeme.lexeme == '!=':
                exprNode.addBody('<>')
            else:
    ...
    def __parseOutput(self, treeNode, parentNode):
        result = OutputNode(parentNode)
        result.addBody('writeln(' + treeNode.lexeme.lexeme + ');\n')
        return result
    ...
    def __parse(self, treeNode, parentNode):
        ...
        elif treeNode.rule.name == '<выводимые данные>':
            return self.__parseOutput(treeNode, parentNode)
        else:
            result = []
            for child in treeNode.children:
    ...
    def __generateIf(self, node: IfNode, level):
        ...
        result += INDENT * (level + 1) + 'begin\n'
        for item in node.body:
            result += self.__generateCode_Helper(item, level + 2)
        result += INDENT * (level + 1) + 'end;\n'
    ...
    def fixIfElse(self, result):
        i_prev = 0
        while i_prev > -1:
            i = result.find('else', i_prev)
            result = result[:result.rfind(';', i_prev, i)] + result[result.rfind(';', i_prev, i) + 1:]
            i_prev = i
            i += 4
        return result
